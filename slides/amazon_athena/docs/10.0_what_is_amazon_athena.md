...
# 2. What kind of service is that?
...
# 4. What Kind of Data Athena Analyzes?
...
SQL clients, connected via a JDBC 
driver.
```

# 7. Back-End Structure of Amazon Athena
...
If you cancel a query, you will be 
charged only for the scanned data up 
to that point.   Of course, you can 
reduce costs by using columnar formats, 
compression, and partitions.  With all 
such techniques, Athena scans fewer 
data from Amazon S3.

It is simple to calculate the charges 
for AWS Athena as it is based on the 
amount of data that needs to be analyzed. 
It improves the performance of the query 
and helps organizations to save cost by 
converting  the  data  to  the  columnar 
formats by using open-source tools such 
as Apache Parquet and Apache ORC. 
```
