...
- **Extract insight** from data sets that are too large, fast, or
  varied for a single machine or traditional software tools
...
- Data sets of billions of elements — measured in **terabytes** and
  **petabytes**
...
- DNA & genomics data
- Twitter feeds, Facebook messages
...
- Data sets now frequently exceed:
  - **1 KB = 1,024 Bytes**
  - **1 MB = 1,024 KB**
  - **1 GB = 1,024 MB**
  - **1 TB = 1,024 GB**
...
**Example — credit cards:** <br>
369 billion purchase transactions worldwide in 2018.

**Example — DNA samples:** <br>
1,000,000 samples × 5,000,000 records each <br>
...
Examples: text messages, Twitter messages, Facebook posts, sensor
...
## Data File Formats: Row vs. Columnar

![](images/08_data_in_rows_and_in_columns.png)
...
- **Horizontal scaling (scale out)** — add *more machines* to a
  ...
  - 10's, 100's, or 1,000's of commodity servers
...
Splitting a large dataset across nodes/files so no single one holds
it all:

- **Hash partitioning** — `hash(key) % N` picks the partition;
  spreads load evenly, but range queries must hit every partition
- **Range partitioning** — contiguous key ranges per partition
  (e.g., by date); great for range queries, risks hot spots
- **List / value partitioning** — one partition per discrete value
  (e.g., by `continent`, `country`) — what Athena/S3 partitioning
  uses (covered later)
...
- **Compliance** — regulatory requirements on sensitive data
  (GDPR, HIPAA for health/genomics data, CCPA)
...
Dedicated ingestion tools:

- **Apache Sqoop** — bulk-load from relational DBs into a Big Data
  system
- **Apache Flume**, **Apache Chukwa** — log/event collection
- **Apache Kafka** — high-throughput streaming ingestion
- **Gobblin**
...
## 4. Computing & Analyzing Data — Two Strategies
...
- Often paired with **in-memory computing** to avoid disk round-trips
...
- **Serverless:**
  ...
  - **Pay-per-query** — you pay only for data scanned
    (~$5 per TB scanned)
- Built on **PrestoDB** (Facebook's distributed SQL engine)
...
- **Note:** no support for multi-statement transactions
...
Ask before reaching for Hadoop/Spark/Athena:

- Does the problem require handling **variety** of data types?
- Does it require handling high **velocity**?
- Does it require handling high **volume**?
- Can the solution handle the **complexity**?
- How can the solution be **optimized** for cost and speed?
